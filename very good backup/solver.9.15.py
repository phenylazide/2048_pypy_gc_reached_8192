...
def gc_eva_score(x):  #x is board ## right,down move is same, no difference. should apply this symmetry in scoring.
	...
	if x[3][1] >= 512 and x[3][0] >= 128:  ## change field  ## left corn in right order:
	...
	for i in range(3,-1,-1):  # value is: 3,2,1
		...
		if i>1 and x[i][j] != 0:
			for j1 in range(4):
				if x[i][j] == x[i-1][j1]: 
					#repeat_scores += x[i][j]*((field[i-1][j1]) + (field[i][j]))
	...
	if x[3][3]>=2048 and x[3][0] <= 128 and x[3][0] > x[3][1]:
		repeat_scores += (200-x[3][0])*20   ## add 200, and substract x[3][0] which added before, so total is 200
                                        ## that is mean FIX x[3][0], use 200 power to drag down same number.
		order_scores = 100*60*get_zhishu_dict[x[3][0]] - x[3][0]*60
		#if x[1][2] > x[1][3]:
...
def gc_solveGame(runs, screen):
	...
	"""
	mainGame = makeGame()
	#mainGame.board = [[0,0,0,2,],[0,0,0,2,],[0,0,0,0,],[2,8,256,2048]]
	moveList = mainGame.moveList
	isDynamic = False
