...
#field = [[0, 0, 1, 2], [0, 1, 4, 8],[1, 5, 15, 20], [3, 12, 25, 50]] 
field = [[0, 0, 1, 2], [0, 1, 2, 4], [3, 5, 8, 12], [15, 20, 25, 50]]   
...
def gc_draw_record_Board(board,screen,gc_eva_score,gameScore):
...
def gc_score_direct_run(board, firstMove, maxDepth):   
	...
	if dGame.gameOver() or first_board == board:
		return (-1,-100)
	moveList = up_movable(first_board)[1]
	for _a in moveList:
		...
		if dGame.gameOver() or (after_a_board == first_board):
			continue
		moveList = up_movable(after_a_board)[1]
		for _b in moveList:
			...
			if dGame.gameOver() or (after_b_board == after_a_board):
				continue
			moveList = up_movable(after_b_board)[1]
			for _c in moveList:
				...
				if dGame.gameOver() or (after_c_board == after_b_board):
					continue
				moveList = up_movable(after_c_board)[1]
				for _d in moveList:
					...
					moveList = up_movable(after_d_board)[1]
					for _e in moveList:
						...
						if dGame.gameOver() or (after_e_board == after_d_board):
							...
							#best_gc_score += new_score
						moveList = up_movable(after_e_board)[1]
						for _f in moveList:
							...
							if dGame.gameOver() or (after_e_board == after_d_board):
								...
								#new_score = gc_eva_score(dGame.board)
									...
									#best_gc_score = new_score
							moveList = up_movable(after_f_board)[1]
							for _g in moveList:
...
def gc_eva_score(x):  #x is board ## right,down move is same, no difference. should apply this symmetry in scoring.
	...
	## i.e.  compare above row
					repeat_scores += int(x[i][j]*field[i-1][j1])*2*abs(j-j1)     ## apply heavy punished
...
## make more blank as many as possible, high power then just pull together, this will encourage merged
	if x[2][2] > x[2][3] and x[1][3] > x[2][3]:
		repair_right_score += (get_zhishu_dict[x[2][2]] + get_zhishu_dict[x[1][3]] - get_zhishu_dict[x[2][2]])*4*x[2][2]
		#if x[1][2] > x[1][3]:
...
def gc_solveGame(runs, screen):
	...
	"""
	mainGame = makeGame()
	#mainGame.board = [[0,0,0,2,],[0,0,0,2,],[0,0,128,0,],[2,8,2048,4096]]
	moveList = mainGame.moveList
	isDynamic = False
