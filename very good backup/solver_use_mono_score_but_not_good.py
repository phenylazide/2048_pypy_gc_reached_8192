...
def gc_eva_score(x):  #x is board ## right,down move is same, no difference. should apply this symmetry in scoring.
	...
	for i in range(3,-1,-1):  # value is: 3,2,1
		for j in range(3,-1,-1):
			if j>0:
				if i>0:
					order_scores += int((1/(abs(get_zhishu_dict[x[i][j]] - get_zhishu_dict[x[i][j-1]]) + 1) + \
															1/(abs(get_zhishu_dict[x[i][j]] - get_zhishu_dict[x[i-1][j]]) + 1) + 1\
															)*x[i][j]*field[i][j])
				else:  ## i = 0
					order_scores += int((1/(abs(get_zhishu_dict[x[i][j]] - get_zhishu_dict[x[i][j-1]]) + 1) + 1\
				             			) *x[i][j]*field[i][j])					
