...
class Engine:
    ...
    def rotateBoard(self, board, count):    ### gc opt this part with the aim to improve perfermance
        ...
        if count == 3: ## clockwise or right rotate 90 degree               ## right move,used most often
        ...
        elif count == 0:  ## anticlockwise or left rotate 90 degree
            for row in range(self.size):
                for col in range(self.size):
    ...
    def addRandBlock(self, val=None):
        ...
        self.spots=avail
        if avail:
            _x = random.randint(0, len(avail) - 1)
            (row, column) = avail[_x]
            del self.spots[_x]
